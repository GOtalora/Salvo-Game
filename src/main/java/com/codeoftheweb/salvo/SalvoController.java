...
public class SalvoController {
    ...
    public Map<String, Object> getAllGames( Authentication authentication) { // Clase de spring que me dice quien esta loggueado
    ...
    public ResponseEntity<Map<String, Object>> createGame( Authentication authentication) { // Clase de spring que me dice quien esta loggueado
        ResponseEntity<Map<String, Object>> respuesta;
        if(isGuest(authentication))
            respuesta = new ResponseEntity<>(makeMap("error", "you are not logged in"),HttpStatus.UNAUTHORIZED);
        else{
            ...
            respuesta = new ResponseEntity<>(makeMap("idGp", gamePlayer.getId()),HttpStatus.CREATED);
        }
        return respuesta;
    }
    ...
    public ResponseEntity<Map<String, Object>> joinGame (Authentication authentication, @PathVariable long gameId){
        ...
        if (isGuest(authentication)){
            ...
            if (game == null){
                respuesta = new ResponseEntity<>(makeMap("error", "no such game"), HttpStatus.FORBIDDEN);
            }else if (game.getGamePlayer().size() > 1){
                respuesta = new ResponseEntity<>(makeMap("error", "Game is full"), HttpStatus.FORBIDDEN);
            }else {
                Player player = playerRepository.findByUserName(authentication.getName());
                if(game.getGamePlayer().stream().anyMatch(gp -> gp.getPlayer().getUserName().equals(player.getUserName()))){
                    respuesta = new ResponseEntity<>(makeMap("error", "you can't play againtst yuourself"),HttpStatus.FORBIDDEN);
                }else {
    ...
    @RequestMapping("/game_view/{gamePlayerId}")
        public ResponseEntity<Map<String, Object>> getGameView(@PathVariable long gamePlayerId, Authentication authentication){
            ...
            if(isGuest(authentication)){
                ...
                Player player = playerRepository.findByUserName(authentication.getName());
                if(gamePlayer ==  null){
                    respuesta = new ResponseEntity<>(makeMap("error", "no such game"), HttpStatus.UNAUTHORIZED);
                }else if (!gamePlayer.getPlayer().getUserName().equals( player.getUserName())){
                        respuesta = new ResponseEntity<>(makeMap("error", "this game is not yours"), HttpStatus.UNAUTHORIZED);
                }else {
                    respuesta = new ResponseEntity<>(gameViewDTO(gamePlayer), HttpStatus.OK);
                }
            }
            return respuesta;
        }
    ...
    @RequestMapping(path = "/players", method = RequestMethod.POST) // ingresa a la bdd y la modifica, RE, devuelve una respuesta
        public ResponseEntity <Map<String, Object>> register(@RequestParam String userName, @RequestParam String firstName, @RequestParam String email, @RequestParam String password){
            ...
            if((userName.isEmpty() || userName.contains(" ") )|| firstName.isEmpty() || email.isEmpty() || (password.isEmpty()|| password.contains(" "))){
                respuesta = new ResponseEntity<>(makeMap("error" , "Missing data"), HttpStatus.FORBIDDEN);
            }else if (player != null){
                respuesta = new ResponseEntity<>(makeMap("error", "Name already in use"), HttpStatus.FORBIDDEN);
            }else{
                Player newPlayer = playerRepository.save(new Player(userName, firstName, email, passwordEncoder.encode(password)));
                respuesta = new ResponseEntity<>(makeMap("userName",newPlayer.getUserName()), HttpStatus.CREATED);
            }
            return respuesta;
        }
    ...
    @RequestMapping(path = "/games/players/{gamePlayerId}/ships", method = RequestMethod.POST)
        public ResponseEntity <Map<String, Object>> addShip(Authentication authentication, @PathVariable long gamePlayerId, @RequestBody List<Ship> ships){
            ...
            if(isGuest(authentication)){
                ...
                if(gp == null){
                    ...
                    respuesta = new ResponseEntity<>(makeMap("error", "you already have ships"),HttpStatus.FORBIDDEN);
                }else if(ships.size() != 5){
                    respuesta = new ResponseEntity<>(makeMap("error", "you must have 5 ships"), HttpStatus.FORBIDDEN);
                }else {
                    if (ships.stream().anyMatch(ship -> isOutOfRange(ship))) {
                        respuesta = new ResponseEntity<>(makeMap("error", "your ships are out of range"), HttpStatus.FORBIDDEN);
                    }else if (ships.stream().anyMatch(ship ->isNotConsecutive(ship))){
                        respuesta = new ResponseEntity<>(makeMap("error", "your ships aren't consecutives"), HttpStatus.FORBIDDEN);
                    }else if (overlappingShips(ships)){
                        respuesta = new ResponseEntity<>(makeMap("error", "your ships are overlapping"), HttpStatus.FORBIDDEN);
                    }else {
                        ships.stream().forEach(ship -> gp.addShip(ship));
                        gamePlayerRepository.save(gp);
                        respuesta = new ResponseEntity<>(makeMap("OK", "adding ships"), HttpStatus.OK);
                    }
                }
            }
            return respuesta;
        ...
        private boolean isNotConsecutive(Ship ship){
            ...
            for (int i = 0; i < cell.size() - 1; i++){
                if(cell.get(i).charAt(0) == cell.get(i+1).charAt(0)){ // es horinzontal
                    Integer y = Integer.parseInt(cell.get(i).substring(1)); //guardo el numero de cada ubicacion
                    Integer yNext = Integer.parseInt(cell.get(i+1).substring(1));
                    if(yNext - y != 1)
                        return true;
                }else { //posiblemente vertical
    ...
    public ResponseEntity <Map<String, Object>> addSalvoes(Authentication authentication, @PathVariable long gamePlayerId, @RequestBody List<String> salvoes){
        ...
        if(isGuest(authentication)){
            ...
            if(gp == null){
                ...
                respuesta = new ResponseEntity<>(makeMap("error", "you must have 5 salvoes"), HttpStatus.FORBIDDEN);
            }else {
                    int turno = gp.getSalvos().size() + 1;
                    Salvo salvo = new Salvo(turno, salvoes);
                    gp.addSalvo(salvo);
                    gamePlayerRepository.save(gp);
                    respuesta = new ResponseEntity<>(makeMap("OK", "adding salvoes"), HttpStatus.OK);
            }
        }
        return respuesta;
    }
