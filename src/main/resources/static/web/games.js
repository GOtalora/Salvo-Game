const app = new Vue({
    ...
    methods:{
            ...
            this.gamePlayers.forEach(gp =>{
                if(gp.score != null){
                    if(!auxPlayerControl.includes(gp.player.id)){
                        ...
                        aux = app.bringPlayer(gp.player.id);
        ...
        signIn(){ //muestra todo una vez logueado
            if(app.user != 'Guest'){
            document.getElementById('userNow').innerHTML = `<h2>Bienvenido : ${this.user.userName}</h2>`
        ...
        viewGame(gamePlayers){
            if(gamePlayers[0].player.userName == app.user.userName)
              location.href = '/web/game.html?gp='+gamePlayers[0].idGp;
            else
                location.href = '/web/game.html?gp='+gamePlayers[1].idGp;
        },
        tableGame(){
            let texto = 
        ...
        viewButton( idGame, playerStartCreator, gamePlayersGame){
            ...
            let texto = "";
            if(gamePlayersGame[0].score != null){// ya se jugo
                texto += `<p>Game Over</p>`
            ...
            }else if(gamePlayersGame.length == 1){ //aun No se termino el juego y solo hay un jugador(para verlo deberia ser el creador)
            ...
            }else{ //aun NO se termino el juego pero estan los dos jugadores
                if(gamePlayersGame[0].player.userName == user.userName || gamePlayersGame[1].player.userName == user.userName)
                    texto += `<button type="button" @click = viewGame(user.id)> View Game</button>`
                else
                    texto += `<h2>In course!</h2>`;
            }
